...
<div class="sideright">
    ...
    <label style="color:black;margin-right:7px;margin-top:-30px;"><br><?php echo $_SESSION['studusername']; ?> </label><br>
    ...
    $s= "select distinct class,module,date from recordattend where username='$uname' ORDER by date";
    $result= mysqli_query($conn,$s);
    echo "<table id='classmodule' name='classmodule'><tr><th>Class</th><th>Module</th><th>Date</th></tr>";
    while($row = $result->fetch_assoc()){
    echo "<tr><td >".$row["class"]."</td><td>".$row["module"]."</td><td>".$row["date"]."</td></tr>";
    } 
    echo "</table>";
    ?>    
    </div>
    ...
    include_once '../dbConnection.php'; 
    $uname =  $_SESSION['studusername'];     
    $s= "SELECT class,COUNT(module) FROM recordattend where username='$uname' GROUP BY class";
    $result= mysqli_query($conn,$s);

    echo "<table id='classmodule' name='classmodule'><tr><th>Class</th><th>Count per Class</th></tr>";
    while($row = $result->fetch_assoc()){
        echo "<tr><td >".$row["class"]."</td><td>".$row["COUNT(module)"]."</td></tr>";   
        } 
